...
def test_mores_mask_is_fixed_first_four_last_five_without_duplicates():
    ...
    mask = build_mores_intervention_mask(config, input_ids)
    ...
    short_mask = build_mores_intervention_mask(
        config,
        torch.tensor([42, 3, 42, 42]),
    )
    ...
    forward_batch = method.build_forward_batch(
        {
            ...
            "intervention_mask": runtime_mask,
        }
    )
    output = model(**forward_batch)

    assert "intervention_mask" not in forward_batch
...
def test_mores_eval_prefill_builds_mask_and_decode_token_is_not_steered():
    ...
    assert method.hook_intervention_tokens == 2
...
def test_mores_compact_checkpoint_has_only_fixed_recipe_tensors_and_round_trips():
    ...
    with pytest.raises(ValueError, match="fixed MoReS v1"):
