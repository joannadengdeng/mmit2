...
def test_initial_release_registry_contains_all_fixed_methods():
...
def test_flat_training_config_accepts_mores_lora_for_qwen():
    ...
    assert config.method == "mores_lora"


@pytest.mark.parametrize("method", ["lora+l2t", "lora,l2t", "l2t_lora"])
def test_flat_training_config_rejects_free_form_method_combinations(method):
...
def test_flat_training_config_rejects_fixed_model_mismatches(
    model,
    method,
    supported_model,
):
