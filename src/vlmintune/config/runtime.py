...
def run_remote_module(
    ...
    *,
    ...
    """Run a remote vlmintune module over SSH and stream its stdout back locally."""
    if paramiko is None:
        print(f"[ERROR] SSH {task_label} requires paramiko: pip install 'vlmintune[remote]'")
        sys.exit(1)

    print(f"[vlmintune] Connecting to {ssh_cfg.username}@{ssh_cfg.host}:{ssh_cfg.port}")
    ...
    print(f"[vlmintune] Connected. Starting remote {task_label}...")
    ...
    try:
        ...
        print(f"\n[vlmintune] Remote {task_label} interrupted by user.")
    ...
    client.close()
    print(f"\n[vlmintune] Remote {task_label} complete.")
