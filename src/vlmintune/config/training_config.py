...
def validate(config: TrainingConfig) -> None:
    ...
    elif config.method == "mole" and config.model != "llava15_7b":
        errors.append("method: 'mole' only supports model 'llava15_7b'")
    elif (
        config.method == "vl_adapter"
        and config.model != "qwen25vl_3b_instruct"
    ):
        errors.append(
            "method: 'vl_adapter' only supports model 'qwen25vl_3b_instruct'"
        )
