...
def auto_select_metric(
    ...
    """
    if task_type == "mcq":
        return "exact_match", "MCQ: exact option-letter match"
    if task_type == "yes_no":
        return "exact_match", "Yes/No: exact match"
    ...
    if task_type == "open_vqa":
        if isinstance(ground_truths, list) and len(ground_truths) >= 3:
            return "vqa_accuracy", "VQA v2 soft accuracy (detected at least 3 annotators)"
        return "exact_match", "Exact match (single-answer data would cap VQA soft accuracy at 0.33)"

    # classification (e.g. ImageNet)
    if task_type == "classification":
        return "exact_match", "Classification: exact class-name match"

    # caption or unknown
    return "token_f1", "Caption/unknown task: token F1 is better for longer text comparisons"
