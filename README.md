...
| `mole` | Single-dataset LLaVA-MoLE v1: ordinary attention LoRA, three FFN experts, shared per-block router, token-level top-1 sparse routing, balance loss | **LLaVA only** |
...
| `l2t` | Standalone full-SFT supervision over retained instruction content and answer; vision encoder frozen | Qwen and LLaVA |

`mole` is the fixed single-dataset release implementation. It does not claim the
multi-dataset conflict setting from the original LLaVA-MoLE experiments.
...
```bash
MODEL=llava15_7b METHOD=mole RUN_NAME=textvqa_mole \
bash experiment_setup/paper_benchmark/run_paper_benchmark.sh
```
...
Validate generated configs for all eight methods without loading a model or GPU:
...
On the AutoDL machine, run all Qwen-compatible methods except MoLE with a small
stage first. Every method must train, save, reload, and produce the requested
number of validation predictions before the next method starts:
...
The initial release does not include target sweeps, layer sweeps, method variants,
or multi-dataset benchmark orchestration.
