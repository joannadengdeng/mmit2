"""Run a tiny Colab smoke-test matrix across multiple mmit2 training paths.

This script is designed for path coverage, not metric quality. It intentionally
uses very small train/eval slices so that we can quickly validate:

- inspect-only debug flow
- LoRA full debug flow (train + reload + eval)
- QLoRA Trainer/CLI flow
- DoRA Trainer/CLI flow
- Freeze Tuning debug flow
- L2T (LoRA base) debug flow
- L2T (QLoRA base) Trainer/CLI flow

Typical usage in Colab after cloning the repo and installing deps:

    python examples/colab_smoke_matrix.py --suite quick
    python examples/colab_smoke_matrix.py --suite full
    python examples/colab_smoke_matrix.py --scenario freeze_debug_tiny
"""
...
def _build_command(scenario: Scenario) -> list[str]:
    ...
    if scenario.kind == "debug":
        return [
            sys.executable,
            str(ROOT / "examples" / "colab_lora_debug.py"),
            "--config",
            config_path,
            *scenario.extra_args,
        ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a tiny Colab smoke-test matrix")
    parser.add_argument(
        "--suite",
        default="quick",
    ...
    print("=" * 80)
    print("mmit2 Colab Smoke Matrix")
    print("=" * 80)
