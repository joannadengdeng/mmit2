...
case "$MODEL" in
  qwen25vl_3b_instruct)
    MODEL_PREFIX="qwen"
    MODEL_CACHE_RELATIVE="models--Qwen--Qwen2.5-VL-3B-Instruct"
    ;;
  llava15_7b)
    MODEL_PREFIX="llava"
    MODEL_CACHE_RELATIVE="models--llava-hf--llava-1.5-7b-hf"
    ;;
  *)
    echo "Unsupported model for the dataset stage: $MODEL" >&2
    exit 2
    ;;
esac
...
case "$DATASET_SLUG" in
  vizwiz|vqav2|gqa)
    ...
    DEFAULT_RUN_PREFIX="${MODEL_PREFIX}_${DATASET_SLUG}_px1003520"
    ;;
  *)
    DEFAULT_RUN_PREFIX="${MODEL_PREFIX}_${DATASET_SLUG}"
    ;;
esac
...
MODEL_CACHE="$HF_HUB_CACHE/$MODEL_CACHE_RELATIVE"
if [[ ! -e "$MODEL_CACHE" ]]; then
  echo "$MODEL cache is not visible at $MODEL_CACHE" >&2
  exit 2
fi
...
checkpoint_marker() {
  local method="$1"
  case "$method" in
    lora|qlora|dora|mores_lora|mores_dora|reft_lora) echo "adapter_config.json" ;;
    mores) echo "mores_tuned.pt" ;;
    reft) echo "reft_tuned.pt" ;;
    vl_adapter)
      if [[ "${MODEL:-qwen25vl_3b_instruct}" != "qwen25vl_3b_instruct" ]]; then
        echo "$method is Qwen-only and cannot run with ${MODEL:-unset}." >&2
        return 2
      fi
      echo "vl_adapter_tuned.pt"
      ;;
...
validate_checkpoint() {
  ...
  python - \
    "$metadata_path" "$config_path" "$expected_method" "$MODEL" "$DATASET" \
...
with open(path, "r", encoding="utf-8", errors="replace") as handle:
    for raw_line in handle:
        ...
        if (
            record.get("type") == "data_summary"
            and data.get("kind") == "preprocessing_coverage"
        ):
            coverage_summaries.append(data)

if not coverage_summaries:
    raise SystemExit(f"training log has no preprocessing_coverage data summary: {path}")
skipped = int(coverage_summaries[-1].get("total_skipped", -1))
if skipped != 0:
    raise SystemExit(
