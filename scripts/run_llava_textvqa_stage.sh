...
RUN_PREFIX="${RUN_PREFIX:-llava_textvqa}"
...
MODEL_CACHE="$HF_HUB_CACHE/models--llava-hf--llava-1.5-7b-hf"
if [[ ! -e "$MODEL_CACHE" ]]; then
  echo "LLaVA cache is not visible at $MODEL_CACHE" >&2
  echo "Download llava-hf/llava-1.5-7b-hf before running this script." >&2
  exit 2
fi
...
checkpoint_marker() {
  ...
  case "$method" in
    ...
    vl_adapter)
      echo "METHODS must not contain vl_adapter because the fixed recipe is Qwen-only." >&2
      return 2
      ;;
    mole)
      echo "METHODS must not contain mole for this requested LLaVA run." >&2
      return 2
      ;;
    *)
...
if metadata.get("model_name") != "llava15_7b":
...
checkpoint_files_exist() {
  ...
  local method="$2"
  local marker="$3"
  [[ -f "$checkpoint_dir/$marker" ]] || return 1
  case "$method" in
    lora|qlora|dora)
      [[ -f "$checkpoint_dir/adapter_model.safetensors" \
          || -f "$checkpoint_dir/adapter_model.bin" ]]
      ;;
    *) return 0 ;;
  esac
}
...
validate_eval() {
  ...
  python - "$summary_path" "$predictions_path" "$EVAL_SAMPLES" <<'PY'
import json
import sys

summary_path, predictions_path, expected_count = sys.argv[1], sys.argv[2], int(sys.argv[3])
...
for key, expected in expected_summary.items():
    if summary.get(key) != expected:
        raise SystemExit(
            f"evaluation summary mismatch for {key}: "
...
with open(predictions_path, "r", encoding="utf-8") as handle:
    ...
    for line_number, line in enumerate(handle, start=1):
        ...
        try:
            ...
            raise SystemExit(f"invalid prediction JSON on line {line_number}: {exc}") from exc
        prediction_ids.append(str(record.get("id")))
        scores = record.get("scores") or {}
        for metric_name, value in scores.items():
            numeric_value = float(value)
            if not (numeric_value == numeric_value and abs(numeric_value) != float("inf")):
                raise SystemExit(
                    f"non-finite prediction score line={line_number} "
                    f"{metric_name}={value}"
                )
        line_count += 1
if line_count != expected_count:
    raise SystemExit(f"expected {expected_count} prediction lines, got {line_count}")
if len(set(prediction_ids)) != expected_count:
    raise SystemExit("prediction IDs are missing or duplicated")
...
for method in "${method_list[@]}"; do
  ...
  completion_marker="$experiment_dir/.complete"

  checkpoint_ready=0
  if checkpoint_files_exist "$checkpoint_dir" "$method" "$marker" \
      && [[ -f "$metadata_path" ]] \
      && validate_checkpoint "$metadata_path" "$method"; then
    checkpoint_ready=1
  fi
  ...
  if [[ "$FORCE" != "1" \
        && "$checkpoint_ready" == "1" \
        && -f "$eval_summary" \
        && -f "$predictions_path" ]]; then
    if validate_eval "$eval_summary" "$predictions_path"; then
      printf 'complete\n' > "$completion_marker"
  ...
  if [[ "$FORCE" == "1" || "$checkpoint_ready" != "1" ]]; then
    ...
    echo "REUSE valid checkpoint and resume evaluation: $run_name"
  fi
...
echo "All requested LLaVA methods passed stage n=$STAGE_SAMPLES."
