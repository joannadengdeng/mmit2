...
export RUN_STAMP="${RUN_STAMP:-vizwiz_scienceqa_smoke_$(date +%Y%m%d_%H%M%S)}"
export MODELS="${MODELS:-qwen25vl_3b_instruct llava15_7b}"
export DATASETS="${DATASETS:-vizwiz scienceqa}"
...
echo "VizWiz/ScienceQA smoke matrix:"
echo "  MODELS=$MODELS"
...
bash "$SETUP_DIR/run_paper_benchmark.sh" \
  2>&1 | tee "run_logs/smoke_vizwiz_scienceqa_${RUN_STAMP}.log"
