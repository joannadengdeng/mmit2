...
for method in lora qlora dora mole reft mores vl_adapter l2t; do
  model="$GENERAL_MODEL"
  if [[ "$method" == "mole" ]]; then
    model="llava15_7b"
  elif [[ "$method" == "vl_adapter" ]]; then
...
echo "Strict configuration smoke passed for all eight release methods."
