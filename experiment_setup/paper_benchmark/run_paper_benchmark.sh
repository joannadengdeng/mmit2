...
case "$METHOD" in
  lora|qlora|dora|reft|mores|vl_adapter|l2t|mores_lora|mores_dora|reft_lora) ;;
  *)
    echo "Unsupported METHOD='$METHOD'. Expected one of: lora qlora dora reft mores vl_adapter l2t mores_lora mores_dora reft_lora" >&2
    exit 2
    ;;
esac
...
if [[ "$METHOD" == "vl_adapter" && "$MODEL" != "qwen25vl_3b_instruct" ]]; then
  echo "METHOD=$METHOD requires MODEL=qwen25vl_3b_instruct." >&2
  exit 2
fi
