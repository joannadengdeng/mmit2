...
Each invocation trains exactly one model, one dataset, and one of the eight
registered methods. Method internals are fixed in code; the runner only writes the
eleven public training fields.
...
- `mole` requires `MODEL=llava15_7b`.
- `vl_adapter` requires `MODEL=qwen25vl_3b_instruct`.
- The other six methods support both built-in models.
